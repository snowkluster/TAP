...
def extract_table_data(page_content):
    search_results = []
    table = page_content.find('table', class_='tborder')
    if table:
            ...
            if len(cells) > 1:
                ...
                for span in span_titles:
                    ...
                    if any(keyword in title_text for keyword in ["New posts.", "Hot thread.", "Closed thread"]):
                                        # Extract the date from forum-display__thread-date
                        thread_date_span = row.find('span', class_='forum-display__thread-date')
                        if thread_date_span:
                            date_value = thread_date_span.get_text(strip=True)
                            break  # Stop checking other title spans once the date is found    
                # If date_value is still None, get the last post date
...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.goto('https://breachforums.st/Forum-Cracked-Accounts')
        while True:
